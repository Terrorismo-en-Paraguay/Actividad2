...
public class CuentaDAOImpl {
    ...
    public void insertarCuenta(Cuenta cuenta){
        try (var conexion = dbc.getConnection(); PreparedStatement stmt = conexion.prepareStatement("Insert Into t_cuenta (numero_cuenta, id_cliente, saldo) VALUES (?, ?, ?)");){
    ...
    public void actualizarSaldo(Cuenta cuenta){
        try (var conexion = dbc.getConnection(); PreparedStatement stmt = conexion.prepareStatement("UPDATE t_cuenta SET saldo=? where numero_cuenta='?'");){
    ...
    public float obtenerSaldo(String cuenta){
        float saldo = 0;
        try(var conexion = dbc.getConnection(); Statement stmt = dbc.getConnection().createStatement(); ResultSet resultado = stmt.executeQuery("SELECT saldo FROM t_cuenta where numero_cuenta='"+cuenta+"'")){
            while(resultado.next()) {
